...
function loadProducts(): Product[] {
  try {
    return JSON.parse(localStorage.getItem('artisan_products') || '[]');
  } catch {
    return [];
  }
}
function loadHistory(): HistoryEntry[] {
  try {
    return JSON.parse(localStorage.getItem('artisan_history') || '[]');
  } catch {
    return [];
  }
}
...
function StockProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    localStorage.setItem('artisan_history', JSON.stringify(history));
  }, [history]);
    ...
    setProducts((prev) =>
      prev.map((p) => {
        ...
        if (data.qty !== undefined && data.qty !== p.qty) {
          ...
          setHistory((h) => [
            {
              type: data.qty! > p.qty ? 'add' : 'remove',
              productId: id,
              productName: data.name || p.name,
              qty: diff,
  ...
  const moveStock = useCallback(
    (productId: string, type: string, qty: number, note: string) => {
      setProducts((prev) =>
        prev.map((p) =>
          p.id !== productId
            ? p
            : { ...p, qty: type === 'add' ? p.qty + qty : p.qty - qty }
        )
      );
      setProducts((prev) => {
  ...
  const duplicateProduct = useCallback((id: string, variants: Variant[]) => {
    setProducts((prev) => {
      ...
      const newProds = variants.map((v) => ({
        ...source,
        id: genId(),
        size: v.size || source.size,
        color: v.color || source.color,
        qty: v.qty,
      }));
      const newHist = newProds
        .filter((p) => p.qty > 0)
        .map((p) => ({
          type: 'add',
          productId: p.id,
          productName: p.name,
          qty: p.qty,
          note: 'Dupliqué depuis ' + source.name,
          date: new Date().toISOString(),
        }));
  ...
  const importData = useCallback(
    (incoming: Product[], incomingHistory: HistoryEntry[], mode: string) => {
      if (mode === 'replace') {
        ...
        setHistory((prev) => {
          const keys = new Set(prev.map((h) => h.date + '_' + h.productId));
          return [
            ...prev,
            ...incomingHistory.filter(
              (h) => !keys.has(h.date + '_' + h.productId)
            ),
          ].sort(
  ...
  const getUniqueValues = useCallback(
    (field: string): string[] => {
      const values = products
        .map((p) => (p as unknown as Record<string, string>)[field])
        .filter((v) => v && v.trim());
        ...
        >
          {photo ? (
            ...
            >
              ...
              <small style={{ fontSize: '0.8rem' }}>Cliquez pour ajouter</small>
            </div>
          )}
        </div>
        <input
          ...
          {CATEGORIES.map((c) => (
            <button
              ...
              style={{
                ...
                background: cat === c.key ? 'var(--accent)' : 'var(--surface)',
                color: cat === c.key ? '#fff' : 'var(--text)',
                fontSize: '0.82rem',
                fontWeight: 500,
              }}
            >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={fieldStyle}>
          ...
          <AutocompleteInput
            value={country}
            onChange={setCountry}
            suggestions={getUniqueValues('country')}
        ...
        <div style={fieldStyle}>
          ...
          <AutocompleteInput
            value={color}
            onChange={setColor}
            suggestions={getUniqueValues('color')}
            placeholder="Ex: Bleu"
          />
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={fieldStyle}>
          ...
          <AutocompleteInput
            value={size}
            onChange={setSize}
            suggestions={getUniqueValues('size')}
            placeholder="Ex: M"
          />
        </div>
        <div style={fieldStyle}>
          ...
          <AutocompleteInput
            ...
            suggestions={getUniqueValues('material')}
...
function BarcodeScanner({
  ...
  const scanFrame = () => {
    ...
    if ('BarcodeDetector' in window) {
      ...
      interface BarcodeDetectorInstance {
        detect: (img: HTMLCanvasElement) => Promise<BarcodeResult[]>;
      }
      ...
      // BarcodeDetector non disponible — affiche message
      setError(
        'Scanner automatique non disponible sur cet appareil. Utilisez la saisie manuelle ci-dessous.'
      );
      setScanning(false);
    }
  };
  ...
  const handleDetected = (code: string) => {
    ...
    const byId = products.find((p) => p.id === code);
    if (byId) {
      onFound(byId.id);
      return;
    }
    // Sinon cherche dans le nom
    ...
    >
      ...
      >
              ...
              >
                ...
                <div
                  style={{
                    ...
                    height: 2,
                    background:
                      'linear-gradient(90deg, transparent, #b5762a, transparent)',
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          ...
          {/* Saisie manuelle */}
          <p style={{ color: '#888', fontSize: '0.78rem', marginBottom: 8 }}>
            Ou saisissez la référence manuellement :
          </p>
...
function HomeScreen({ navigate }: { navigate: (r: Route) => void }) {
  ...
  const handleMailExport = () => {
    ...
    const body = encodeURIComponent(
      "Bonjour,\n\nVeuillez trouver ci-dessous l'export du stock artisanat du " +
        date +
        '.\n\n' +
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const prods: Product[] = Array.isArray(parsed)
          ? parsed
          : parsed.products || [];
        const hist: HistoryEntry[] = parsed.history || [];
        if (prods.some((p: Product) => !p.id || !p.name)) {
  ...
  const confirmImport = (mode: string) => {
    ...
    if (
      mode === 'replace' &&
      !confirm('Remplacer tous les produits existants ?')
    )
      return;
    ...
    >
      ...
      >
        <div>
          ...
          >
            Artisanat <span style={{ color: '#c9935a' }}>&</span> Déco
          </h1>
          <p
            style={{
              fontSize: '0.65rem',
        ...
        <div style={{ display: 'flex', gap: 20 }}>
          {[
            ['Produits', products.length, false],
            ['En stock', totalStock, false],
            ...(lowStock > 0 ? [['Stock bas', lowStock, true]] : []),
          ].map(([lbl, num, danger]) => (
            <div key={String(lbl)} style={{ textAlign: 'center' }}>
              <span
                style={{
                  ...
                  fontSize: '1.2rem',
                  fontWeight: 700,
                  color: danger ? 'var(--danger)' : '#c9935a',
                }}
              >
                {String(num)}
              </span>
          ...
          {[{ key: 'all', label: 'Tous' }, ...CATEGORIES].map((c) => (
            <button
              ...
              style={{
                ...
                border: '1.5px solid',
                borderColor:
                  filter === c.key ? 'var(--accent)' : 'var(--border)',
                background:
                  filter === c.key ? 'var(--accent)' : 'var(--surface)',
                color: filter === c.key ? '#fff' : 'var(--text)',
                fontSize: '0.8rem',
                fontWeight: 500,
                whiteSpace: 'nowrap',
              }}
                  ...
                  style={{
                    ...
                    background:
                      h.type === 'add'
                        ...
                        : 'var(--danger-light)',
                    color:
                      h.type === 'add' ? 'var(--success)' : 'var(--danger)',
                    whiteSpace: 'nowrap',
                  }}
                >
                  {h.type === 'add' ? '+ Entrée' : '− Sortie'}
                </span>
                <div style={{ flex: 1 }}>
                  <span style={{ fontWeight: 600 }}>{h.productName}</span>
                  <strong style={{ color: 'var(--accent)' }}> ×{h.qty}</strong>
                  {h.note && (
                    <span
                      style={{
                        color: 'var(--text-muted)',
      ...
      >
        ...
        >
          {products.length} produit(s) exporté(s) · Copiez ce JSON et
          sauvegardez-le dans un fichier .json
        </p>
        <textarea
          readOnly
          value={exportJson}
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              background: exportCopied ? 'var(--success)' : 'var(--accent)',
              color: '#fff',
              fontSize: '0.88rem',
              fontWeight: 600,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              ...
              <span style={{ color: 'var(--text-muted)' }}>
                {importPreview.products.length} produit(s) ·{' '}
                {importPreview.history.length} mouvement(s)
              </span>
            </div>
            <p
              style={{
            ...
            >
              {[
                ['add', '➕', 'Ajouter', "Fusionne avec l'existant"],
                ['replace', '🔄', 'Remplacer', 'Efface tout'],
...
function ProductScreen({
  ...
  if (!product)
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        Produit introuvable{' '}
        <button onClick={() => navigate({ name: 'home' })}>← Retour</button>
      </div>
    );
  ...
  const BarcodeVisual = () => {
    ...
    for (let i = 0; i < product.id.length * 3; i++) {
      const w =
        ((product.id.charCodeAt(i % product.id.length) * (i + 1)) % 3) + 1;
      bars.push(
        <div
          key={i}
          style={{
            width: w,
            background: i % 2 === 0 ? '#2c2416' : 'transparent',
            flexShrink: 0,
            height: 60,
          }}
        />
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: 20, maxWidth: 700, margin: '0 auto' }}>
        ...
        >
          ...
          >
            <span
              style={{ display: 'block', fontSize: '1.8rem', fontWeight: 800 }}
            >
              {product.qty}
            </span>
            <span
        ...
        {product.qty > 0 && isLow && (
          ...
          >
            ⚠ Stock bas — seuil : {product.threshold}
          </div>
        )}
        ...
        >
          {[
            [
              'Catégorie',
              product.cat ? catEmoji[product.cat] + ' ' + product.cat : '',
            ],
            ['Pays', product.country ? '🌍 ' + product.country : ''],
            ['Taille', product.size ? '📐 ' + product.size : ''],
            ['Couleur', product.color ? '🎨 ' + product.color : ''],
            ['Matière', product.material ? '🧵 ' + product.material : ''],
            [
              'Prix',
              product.price
              ...
              >
                ...
                <span
                  style={{
                    fontWeight: 500,
                    textAlign: 'right',
                    fontFamily: label === 'Référence' ? 'monospace' : 'inherit',
                    fontSize: label === 'Référence' ? '0.78rem' : 'inherit',
                  }}
                >
                  {value}
                </span>
        ...
        >
          {[
            ['var(--accent)', '±', 'Mouvement', () => setShowMovement(true)],
            ['#5b4db5', '▦', 'Code-barres', () => setShowBarcode(true)],
            ['#2980b9', '✏️', 'Modifier', () => setShowEdit(true)],
            [
              '#16a085',
              '⧉',
              'Dupliquer',
      ...
      >
        <p style={{ color: 'var(--text-muted)', marginBottom: 14 }}>
          Copie de : <strong>{product.name}</strong>
        </p>
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: '1fr 1fr 65px 28px',
            gap: 6,
            fontSize: '0.7rem',
            fontWeight: 700,
            textTransform: 'uppercase',
        ...
        {variants.map((v, i) => (
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                const nv = [...variants];
                nv[i].qty = parseInt(e.target.value) || 0;
                setVariants(nv);
              }}
            />
            {variants.length > 1 && (
        ...
        <button
          onClick={() =>
            setVariants([...variants, { size: '', color: '', qty: 1 }])
          }
          style={{
            background: 'none',
...
export default function App() {
  ...
  return (
    <StockProvider>
      ...
      {route.name === 'product' && (
        <ProductScreen id={route.id} navigate={setRoute} />
      )}
    </StockProvider>
  );
}
